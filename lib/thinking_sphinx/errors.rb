class ThinkingSphinx::SphinxError < StandardError
  ...
  def self.new_from_mysql(error)
    ...
    when /index not available/
      replacement = ThinkingSphinx::IndexNotFoundError.new(error.message)
    else
...
class ThinkingSphinx::IndexNotFoundError < ThinkingSphinx::QueryError
end
